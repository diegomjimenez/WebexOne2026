...
mcp_client.connect(
    ...
    args=os.getenv("MCP_SERVER_ARGS", "07_agents_server.py").split(","),  
...
#   1. load_skill         — from skills_loader (client-side playbooks)     
#   2. check_webex_status — from local_tools  (a plain HTTP call)          
#   3. prompt__* meta-tools — from the MCP server's prompts (workflows)    
skills = skills_loader.discover(SKILLS_DIR)                               
...
SYSTEM_PROMPT = (
    ...
    + skills_loader.catalog_prompt(skills)                               
    + "\n\n"
